    ...
    else if (1 == run_data->tcp_start)
    {
        ...
        if (ss_client.connected())
        {
            ...
            if (ss_client.available()||data_size>0)
            {
                ...
                if(get_img_statue==0)
                {
                    ...
                    for(int i=0;i<data_size+read_count-1;i++)
                    {
                        if(run_data->recvBuf[i]==0xff&&run_data->recvBuf[i+1]==0xd8)
                        {
                            ...
                            if(img_start>20000)
                            {
                                memcpy(run_data->recvBuf ,run_data->recvBuf+img_start,data_size-img_start);
                ...
                // Serial.printf("P6"); 
                    // if(
                    //     run_data->recvBuf[img_start]==0xff&&
                    //     run_data->recvBuf[img_end]==0xd8&&
                    //     run_data->recvBuf[img_end-1]==0xff&&
                    //     run_data->recvBuf[img_end]==0xd9)//帧头帧尾
                    // {
                        ...
                        Serial.printf("帧大小：%d MCU处理时间：%dms\n", img_end-img_start+1,GET_SYS_MILLIS() - deal_time);
                        // Serial.print("MCU处理速度：");
                        // Serial.print((GET_SYS_MILLIS() - deal_time), 2);
                        // Serial.print("ms\n");
                        Serial.print("实际帧率：");
                        Serial.print(1000.0 / (GET_SYS_MILLIS() - disp_time), 2);
                        disp_time = GET_SYS_MILLIS();
                        Serial.print("Fps\n");
                    // }   
                    return;
        ...
        else
        {
            ...
            // Serial.println(F("debug point3!"));
            unsigned long timeout = GET_SYS_MILLIS();
            // Serial.println(F("debug point4!"));
